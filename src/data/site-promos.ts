import type { SitePromoCampaign } from "@/types/site-promo";

/**
 * Swappable house campaigns. Edit this file (or later Admin) to rotate deals —
 * slot layouts stay fixed.
 *
 * Claim hygiene: prefer softer copy until analytics can back “3× / 25%” lines.
 */
export const SITE_PROMO_CAMPAIGNS: readonly SitePromoCampaign[] = [
  {
    id: "boost-launch-2026",
    priority: 100,
    active: true,
    slotIds: ["specialist_dashboard_boost"],
    audience: "specialist",
    eyebrow: "Grow on SMOAC",
    headline: "Boost your profile where clients look",
    body: "Sponsored, Featured spotlight, and category pins put you in paid discovery — separate from Pro analytics.",
    stat: "Specialists who boost get more profile appearances in Explore and homepage rails.",
    ctaLabel: "Explore boosts",
    ctaKind: "open_boost",
    dismissible: true,
    accent: "neon-yellow",
  },
  {
    id: "pro-trial-once-2026",
    priority: 90,
    active: true,
    slotIds: ["specialist_dashboard_hero"],
    audience: "specialist",
    eyebrow: "SMOAC Pro",
    headline: "Unlock Pro — 1 month free",
    body: "One-time offer. No card required. You get full Pro access for 30 days, then your account returns to Free unless you upgrade.",
    stat: "Available once per specialist account.",
    ctaLabel: "Unlock Pro · 1 month free",
    ctaKind: "claim_pro_trial",
    dismissible: true,
    orbitCta: true,
  },
  {
    id: "pro-upgrade-now-2026",
    priority: 90,
    active: true,
    slotIds: ["specialist_dashboard_pro_upgrade"],
    audience: "specialist",
    eyebrow: "Ready now?",
    headline: "Upgrade to Pro",
    body: "Skip the trial — unlock full analytics, ranking intelligence, and growth insights today.",
    stat: "$19.99/mo · Cancel anytime from billing.",
    ctaLabel: "Upgrade to Pro · $19.99/mo",
    ctaKind: "open_pro",
    dismissible: true,
    orbitCta: true,
  },
];
