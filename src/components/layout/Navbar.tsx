...
export function Navbar() {
  ...
  if (pathname !== routePath) {
    ...
    if (savedPanelOpen) setSavedPanelOpen(false);
  }
  ...
  const toggleMenu = useCallback(() => {
    setMenuOpen((open) => {
      if (!open) setSavedPanelOpen(false);
      return !open;
    });
  }, []);
  ...
  useEffect(() => {
    if (!menuOpen && !savedPanelOpen) return;

    function onKeyDown(e: KeyboardEvent) {
      if (e.key !== "Escape") return;
      if (savedPanelOpen) closeSavedPanel();
      else if (menuOpen) closeMenu();
    }

    window.addEventListener("keydown", onKeyDown);
    return () => window.removeEventListener("keydown", onKeyDown);
  }, [menuOpen, savedPanelOpen, closeMenu, closeSavedPanel]);
  ...
  return (
    <header className="fixed top-0 right-0 left-0 z-50 supports-[padding:max(0px)]:pt-[env(safe-area-inset-top)]">
        ...
        <div className="site-navbar__inner mx-auto flex h-14 max-w-7xl items-center justify-between px-4 sm:h-16 sm:px-6 lg:h-[72px]">
          ...
          <div className="hidden items-center gap-8 md:flex lg:gap-10">
            ...
            <NavbarProfileLink
              ...
              navMenuOpen={menuOpen || savedPanelOpen}
            />
            <Link
              href="/explore"
      ...
      <SavedPanelDropdown open={savedPanelOpen} onClose={closeSavedPanel} />
