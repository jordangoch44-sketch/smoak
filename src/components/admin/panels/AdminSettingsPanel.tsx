...
export function AdminSettingsPanel() {
  ...
  useEffect(() => {
    ...
    void fetch("/api/admin/platform-status", { credentials: "include" })
      ...
      .then((body: { ok?: boolean; message?: string } & Partial<PlatformStatus> | null) => {
        if (cancelled) return;
  ...
  return (
    <DashboardSection
      title="Settings"
      description="Live platform status — taxonomy editors will land when Explore needs DB-backed categories."
    >
      {error ? <p className="admin-status-error">{error}</p> : null}
      {!status && !error ? <p className="admin-empty">Loading status…</p> : null}
      {status ? (
        <div className="admin-placeholder-grid">
          ...
          <div className="admin-placeholder-card">
            <h3>Supabase</h3>
            <p>{status.supabaseConfigured ? "Connected" : "Not configured"}</p>
          </div>
          <div className="admin-placeholder-card">
          ...
          <div className="admin-placeholder-card">
            <h3>City rankings</h3>
            <p>
              {status.ranking.source}. Sort: {status.ranking.formula}. Excludes:{" "}
              {status.ranking.excludes.join(", ")}.
            </p>
          </div>
        </div>
      ) : null}
