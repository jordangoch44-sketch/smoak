...
export function StatTile({
  ...
  return (
    ...
    >
      ...
      <div className="dashboard-stat-tile__values">
        <PremiumLockedValues locked={lockValues}>
          <p className="dashboard-analytics-stat__value">{value}</p>
          {detail ? (
            <p className="dashboard-analytics-stat__detail">{detail}</p>
          ) : null}
        </PremiumLockedValues>
      </div>
      {lockValues ? (
        <div className="dashboard-stat-tile__lock-veil" aria-hidden />
      ) : null}
    </div>
  );
}
