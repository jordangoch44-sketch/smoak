...
export async function GET() {
  ...
  if (!profileId) {
    return NextResponse.json({
      ok: true,
      counts: { profileViews: 0, savedByClients: 0 },
    });
  }
  ...
  if (visitsRes.error) {
    console.warn("[SMOAC analytics] site_visits count failed:", visitsRes.error.message);
  }
  if (savesRes.error) {
    console.warn("[SMOAC analytics] saved_trainers count failed:", savesRes.error.message);
  }
